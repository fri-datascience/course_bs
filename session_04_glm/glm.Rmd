...
A very useful modelling technique in Bayesian statistics are so called robust models. Robustness here depicts that they are less sensitive to outliers or extreme data values. In modelling, we usually achieve this by modelling errors (spread) with a distribution that has higher tails. Such a distribution will not shift the mean towards extreme values so much during the fitting process in order to assign some non-negligable likelihood to these points with extreme values. Probably the most common technique is to use the Cauchy distribution instead of a normal one. An alternative approach is to remove outliers from your data, but that process is almost always tricky as you usually need to resort to some arbitrary thresholds which classify points as outliers or not. It is often much more elegant and objective to simply use a model that is robust to these outliers. Below is a robust variant of the simple linear regression described above:

$$y | x, \alpha, \beta, \sigma \sim Cauchy(\alpha + x \beta, \sigma).$$

Stan code for this model can be found below.
...
The figure below sees the application of this model, in comparison with the standard linear regression on an artifical dataset with a clear cluster of outliers. The visualization nicely showcases how the robust model is not influenced by this cluster as much as the standard model.

<center>
  ![](./figs/linear_robust.png)
</center>

Note that this technique does not apply only to regression model. Probably the most commonly used Bayesian robust model is the robust t-test, where the normal distribution is again replaced with the Cauchy distribution.

## Simple linear regression with a variable sigma

In the vast majority of our models we will investigate how the mean of some resposne variable (y) changes with exploratory variables (x). But there might be cases where mean is not the only thing that changes with x, in the example above mean is constant while variance changes with x. So in this case, we need to "linearize" the $\sigma$ in the simple linear regression above. Again, this is relatively simple to do and showcased in the model below.

$$y | x, \beta, \sigma \sim N(\mu, x \beta).$$
...
The figure below showcases this model applied to a dataset where the mean is constant, while the variance changes with x.

<center>
  ![](./figs/linear_sigma.png)
</center>

Note that there are some caveats to this approach. If you were to investigate y outside of the dataset's domain ([1, 10] in our example case), you might encounter the issue of negative sigmas, which obvisouly will not work. There are solutions to that, instead of a linear function for sigma, you could use a function that asymptotically approaches 0.

## Multiple linear regression

Multiple linear regression is a logical next step after getting to know the simple linear regression model. The only difference between the two is that multiple linear regression can handle input data that has more than one idependent variable. As a result, we can check the correlation between the dependant variable ($y$) and many idependent variables ($x_1, x_2, \ldots, x_k$) simultaneously. The upgrade is quite easy. First, we have a new entry in the input data block, where $m$ denotes the number of independent variables. Second, our input variables are now stored in a matrix ($X$), since there are many of them. And third, we have multiple $\beta$ coefficients, one for each independent variable. Below is an example of multiple linear regression as a .stan model:
...
$$y \; | \; x_r, x_a, x_m, \alpha, \beta_r, \beta_a, \beta_m, \sigma \sim N(\alpha + \beta_r x_r + \beta_a x_a + \beta_m x_m, \sigma),$$
...
$$y \; | \; x_r, x_a, x_m, \beta_r, \beta_a, \beta_m, \sigma \sim N(\beta_r x_r + \beta_a x_a + \beta_m x_m \sigma).$$
...
$$y \; | \; x_r, \beta_3, \beta_2, \beta_1, \sigma \sim N(\beta_3 x_r^3 + \beta_2 x_r^2 + \beta_1 x_r, \sigma).$$
...
$$y \; | \; x_h, x_w, \alpha, \beta_h, \beta_w, \beta_{hw}, \sigma \sim N(\alpha + \beta_h x_h + \beta_w x_w + \beta_{hw} x_h x_w, \sigma).$$
...
$$y \; | \; x, \alpha, \beta \sim \text{Bernoulli}(\frac{1}{1 + e^{-(\alpha + x  \beta)}}).$$
...
Since the above example might be a bit overwhelming there is also a simpler one in the code examples (`code/10_categorical_simple.R`). In there were use longitude and hemishpere data to predict the probability that a given point lies on one of the continents. The interpretation of beta coefficients there is much more easy to inrepret as it is very obvious how probability of being on a certain continent changes as we move around the planet.
...
$$E[y] = \mu = \frac{\nu}{\lambda},$$
$$\text{Var}(y) = \phi = \frac{\nu}{\lambda^2}.$$
As you can see we cannot directly map the linear predictor to a gamma parameter. Fear not, we can fix this with some simple calculus. We can reparametrize the gamma distribution to fit our needs. There are two ways this is commonly done in the literature. The first:

$$\mu = \frac{\nu}{\lambda},$$
$$\nu = \mu\lambda.$$
Now we have an expected value parameter $\mu$. Since gamma distribution resides on the positive real scale we can use a log link function. Putting everything together gets us:

$$y \; | \; \mu, \lambda \sim \text{gamma}(\mu\lambda, \lambda),$$
...
Here, we again have an expected value parameter $\mu$ and also a variance parameter $\phi$. We can again use a log link:

$$y \; | \; \mu, \phi \sim \text{gamma}(\frac{\mu^2}{\phi}, \frac{\mu}{\phi}),$$
$$\text{log}(\mu) = \beta_0 + \beta_1 x_1 + \ldots + \beta_k x_k,$$
$$\mu = e^{\beta_0 + \beta_1 x_1 + \ldots + \beta_k x_k}.$$

Based on the above, you should now be able to develop a gamma regression model in Stan.
