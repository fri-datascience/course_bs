...
df <- read.table("../data/SO2.csv", h = T, sep = ";")
df$Type <- "Raw"
...
for (i in idx) {
  ...
  # ssn
  ssn1 <- df_s$`beta_cos[1]`[i] * cos(omega[1] * x) + df_s$`beta_sin[1]`[i] * sin(omega[1] * x)
  ssn2 <- df_s$`beta_cos[2]`[i] * cos(omega[2] * x) + df_s$`beta_sin[2]`[i] * sin(omega[2] * x)
  ssn <- ssn1 + ssn2
  df_decomposed <- df_decomposed %>%
    add_row(data.frame(idx = as.character(i),
                       Type = "Seasonality",
