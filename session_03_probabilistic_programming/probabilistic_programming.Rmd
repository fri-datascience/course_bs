...
$$y_1, \ldots, y_n \qquad y_i \in {0,1}.$$

2. The model:

$$y_1, \ldots, y_n | \theta \sim_{iid} Bernoulli(\theta).$$
...
The above diagnostics will be enough for the majority of our use cases, Stan also offers a function ($\texttt{cmdstan_diagnose()}$) that automatically performs some essential checks and lets you know if some things are not in order. If the name of your model fit variable is $\texttt{fit_normal}$ then running $\texttt{fit_normal\$cmdstan_diagnose()}$ woud run the basic diagnostics on it. Furthermore, you can use the $\texttt{shinystan}$ package to open up a web interface in which you can interactively explore your fits.
