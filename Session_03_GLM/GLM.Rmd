...
$$\text{softmax}(j, x_1, x_2, \ldots, x_n) = \frac{e^{x_j}}{\sum_{i=1}^{n} e^{x_i}},$$

wjere $j \in [1 .. k]$. Note here that $\beta$ is not a vector (or a scalar) but a matrix. If we have $k$ outcomes of our dependent variable $y$ and $n$ observations of $m$ independent variables, then $\beta$ will be a matrix with $k$ rows and $m$ columns.
