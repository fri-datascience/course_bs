...
where $\Phi$ is the cumulative density function of the standard normal distribution $Z$. And we can derive the probability that $|x|$ is smaller than some threshold $y$ as:

$$Pr(|X| < y) = Pr(-y < X < y),$$

$$Pr(-y < X < y) = Pr(X < y) - Pr(X < -y),$$

$$Pr(|X| < y) = \Phi(\frac{y - \mu}{\sigma}) - \Phi(\frac{-y - \mu}{\sigma}).$$

In the special case, where $\mu = 0$, we can simplify:

$$\Phi(-a) = 1 - \Phi(a),$$

$$\Phi(\frac{-y}{\sigma}) = 1 - \Phi(\frac{y}{\sigma}),$$

$$Pr(|x| < y) = 2 \Phi(\frac{y}{\sigma}) - 1.$$

Putting it all together for our problem this becomes:
...
$$y \; | \; x, n, p \sim \text{binomial}(n, p),$$
$$p = 2 \Phi \left ( \frac{\alpha}{\sigma} \right ) - 1 = 2 \Phi \left ( \frac{tan^{-1} \left ( \frac{R-r}{x} \right )}{\sigma}  \right ) - 1,$$
...
$$y \; | \; x, n, p \sim \text{binomial}(n, p),$$
$$p = \Phi \left ( \frac{d_t - o}{(x + o) \sigma_d} \right ) - \Phi \left ( \frac{- o}{(x + o) \sigma_d} \right ),$$
$$\sigma \sim \text{Cauchy}(0, 2.5).$$
...
$$y \; | \; x, n, p \sim \text{binomial}(n, p),$$
$$p_a = 2 \Phi \left ( \frac{tan^{-1} \left ( \frac{R-r}{x} \right )}{\sigma_a}  \right ) - 1,$$
$$p_d = \Phi \left ( \frac{d_t - o}{(x + o) \sigma_d} \right ) - \Phi \left ( \frac{- o}{(x + o) \sigma_d} \right ),$$
$$p = p_a \; p_d,$$
