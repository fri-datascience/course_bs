...
The `50_startups.csv` dataset contains information about how many resources a startup company used for research, administration or marketing along with the location of the company. Upgrade the linear regression model we developed during lectures into a multiple linear regression model and present the results in a single graph. The purpose of this task is to install Stan and get familiar with its basics, so do not bother with "more advanced" (G)LM modelling techniques, such as predictor interactions.
