...
If we insert the above Bernolli likelihood and beta prior into the Bayes' rule we get:
...
In the Bayesian setting a probabilistic program is a description of how to compute the posterior distribution. So, ironically a probabilistic model (a probabilistic program) is usually not something indeterministic.
...
No we can compare how this basketball player fares when shooting on a default sized rim in comparison to a smaller rim (the special rim). To do this we just have to compare the model's \texttt{theta} parameter between both cases. Here we are using the \texttt{mcse} library because it conveniently also calculates the Monte Carlo standard error. It is important to always estimate uncertainty in our conclusions!

The first comparison calculates the difference in success between the default and the special rims. We can see that the player has a 30% ($\pm$ 0.2%) higher success rate when shooting on the default rim. The second comparison calculates the probability with which we can claim that player's success is indeed higher when shooting on the default rim. As you can see we can claim this with 99.97% ($\pm$ 0.02%) probability. So we are extremely confident that shooting on a larger rim will yield better success. The code snippet below visualizes this finding.
...
In the following example we will use Bayesian linear regression to model some data from a toy (`toy.csv`) dataset. The dataset contains two columns, `x` denotes the independent and `y` the dependent variable. We will use a linear normal model (or linear normal regression) to model this dependency. At the same time we will see how one would build a model for normally distributed data. This is the Stan model we will use:
...
The `traceplot` and `summary` both show no reasons for any concerns, we will skip them here for brevity, but in practice you should always start your analysis by inspecting the outputs of these two functions. Whenever possible you should also always visually inspect how fitted distributions match the underlying data. With more complex and highly dimensional models this most likely will not be possible but here we can easily do this.
...
* Wiki page on statistical models (https://en.wikipedia.org/wiki/Statistical_model).

* Wiki page on probabilistic programming (https://en.wikipedia.org/wiki/Probabilistic_programming).
...
* If you did not do this already, install a Stan interface. Our examples and lectures will be based on CmdStanR (https://mc-stan.org/cmdstanr/), if you wish, you can use a different one though (CmdStanPy, RStan, PyStan ...). To verify the installation run the corresponding example (e.g. https://mc-stan.org/cmdstanr/articles/cmdstanr.html for CmdStanR).
* Download the `50_startups.csv` dataset from course's repository (https://github.com/fri-datascience/course_bs).

## The problem

We are owners of a startup in the USA and we have to decide how to distribute our resources and where to open our offices. The `50_startups.csv` dataset contains information about how many resources a startup company used for research, administration or marketing along with the location of the company and their profit. Upgrade the linear regression model we developed during lectures into a multiple linear regression model and use it to gain insight into how we should distribute our resources and where we should open our offices to maximize our profit.

The purpose of this task is to install Stan and all the supporting libraries required for executing a Bayesian analysis. Right now our goal is to get familiar with modelling basics, so do not bother with "more advanced" (G)LM modelling techniques, such as predictor interactions. Submit the solution over e-classroom in the form of a single page A4 PDF.
