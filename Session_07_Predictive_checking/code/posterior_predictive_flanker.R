...
library(HDInterval) # for HDI intervals
...
ggplot() +
  ...
  geom_vline(xintercept=1, size = 2, color="grey75", linetype="dashed") +
  theme_minimal() +
...
for (i in 1:nrow(df_exp_i)) {
  mu <- df_exp_i[i,]$mu
  sigma <- df_exp_i[i,]$sigma
  lambda <- df_exp_i[i,]$lambda
  p_slower_exp_i <- c(p_slower_exp_i, 1 - pemg(1, mu=mu, sigma=sigma, lambda=lambda))
}

# compare
mean_normal <- round(mean(p_slower_normal), 4) * 100
hdi_normal <- round(hdi(p_slower_normal, credmass=0.90), 4) * 100
cat(paste0("Normal model: ", mean_normal,"% (90% HDI ", hdi_normal[1], "-", hdi_normal[2], "%) slower than 1 s"))

mean_emg <- round(mean(p_slower_exp_i), 4) * 100
hdi_emg <- round(hdi(p_slower_exp_i, credmass=0.90), 4) * 100
cat(paste0("EMG model: ", mean_emg,"% (90% HDI ", hdi_emg[1], "-", hdi_emg[2], "%) slower than 1 s"))
